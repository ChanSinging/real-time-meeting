...
export default class RtcClient {
  ...
  async checkNetworkQuailty() {
    const { tokens, appId, roomId } = this.config
    // const [] = tokens;
    const uplinkClient = this.engine.createEngine(appId);
    const downlinkClient = this.engine.createEngine(appId);
    await uplinkClient.startAudioCapture();
    await uplinkClient.startVideoCapture();
    // uplinkClient 进房，设置为自动发布，开启音视频采集
    uplinkClient.joinRoom(tokens[0].token, roomId, { userId: tokens[0].userId }, {
      isAutoPublish: true,
    ...
    downlinkClient.joinRoom(tokens[1].token, roomId, { userId: tokens[1].userId }, {
      isAutoPublish: false,
      isAutoSubscribeAudio: true,
      isAutoSubscribeVideo: true
    });
